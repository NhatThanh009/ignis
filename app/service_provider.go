...
func (s *serviceProvider) PGPool(ctx context.Context) *pgxpool.Pool {
	if s.pgPool == nil {
		...
		err = pool.Ping(ctx)
		if err != nil {
			log.Printf("failed to ping database: %s", err.Error())
			pool.Close()
			return nil
		}
...
func (s *serviceProvider) UserRepository(ctx context.Context) db.Repository {
	if s.userRepository == nil {
		pool := s.PGPool(ctx)
		if pool == nil {
			return nil
		}
		s.userRepository = db.NewRepository(pool)
	}
